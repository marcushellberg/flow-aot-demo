...
public final class JsonUtils {
	 ...
	 * <p>
	 * This is a helper for overcoming the fact that {@link JsonValue} doesn't override
	 * {@link Object#equals(Object)} and {@link JsonValue#jsEquals(JsonValue)} is defined
	 * to use JavaScript semantics where arrays and objects are equals only based on
	 * identity.
	 * @param a the first json value to check, may not be null
	 * @param b the second json value to check, may not be null
	 * @return <code>true</code> if both json values are the same; <code>false</code>
	 * otherwise
	 */
	 ...
	 * @param array the JSON array to create a stream from
	...
	/**
	 * Creates a stream from a JSON array of objects. This method does not verify that all
	 * items in the array are actually JSON objects instead of some other JSON type.
	 * @param array the JSON array to create a stream from
	...
	/**
	 * Creates a double stream from a JSON array of numbers. This method does not verify
	 * that all items in the array are actually JSON numbers instead of some other JSON
	 * type.
	 * @param array the JSON array to create a stream from
	 ...
	 * @param values the values that should be in the created array
	...
	/**
	 * Converts the given map into a JSON object by converting each map value to a JSON
	 * value.
	 * @param <T> the type of the map values
	 * @param map the map to convert into a JSON object
	 * @param itemToJson callback for converting map values to JSON
	 ...
	 * @param bean the bean to convert, not {@code null}
	 ...
	 * @param list the list to convert, not {@code null}
	 * @return a JSON representation of the bean
	 */
	public static JsonArray listToJson(List<?> list) {
		Objects.requireNonNull(list, CANNOT_CONVERT_NULL_TO_A_JSON_OBJECT);
		try {
			var valueAsJsonString = objectMapper.writeValueAsString(list);
			logger.info("asString:[" + valueAsJsonString + ']');
			var parse = (JsonArray) Json.instance().parse(valueAsJsonString);
			logger.info("returned object: " + parse);
			return parse;
		}
		catch (JsonProcessingException e) {
			logger.warn("oops!", e);
