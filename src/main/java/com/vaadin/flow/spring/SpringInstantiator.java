 ...
 * This implementation uses Spring beans.
 *
 * @author Vaadin Ltd
 */
public class SpringInstantiator extends DefaultInstantiator {
	...
	public Stream<VaadinServiceInitListener> getServiceInitListeners() {
		Stream<VaadinServiceInitListener> springListeners = context.getBeansOfType(VaadinServiceInitListener.class)
				.values().stream();
			...
			if (loggingEnabled.compareAndSet(true, false)) {
				LoggerFactory.getLogger(SpringInstantiator.class.getName())
						.info("The number of beans implementing '{}' is {}. Cannot use Spring beans for I18N, "
								+ "falling back to the default behavior", I18NProvider.class.getSimpleName(),
								beansCount);
			}
	 ...
	 * <ul>
	 * <li>If exactly one bean is present in the context, it returns this bean.</li>
	 * <li>If no bean is present, it tries to instantiate one.</li>
	 * <li>If more than one bean is present, it tries to instantiate one but in case of a
	 * Bean instantiation exception this exception is catched and rethrown with a hint.
	 * Reason for this is, that users may expect it to "use" a bean but have multiple in
	 * the context. So the hint helps them find the problem.</li>
	 * </ul>
	 */
	@Override
	public <T> T getOrCreate(Class<T> type) {

		System.out.println("***** " + type.getName() + " constructors: ");
